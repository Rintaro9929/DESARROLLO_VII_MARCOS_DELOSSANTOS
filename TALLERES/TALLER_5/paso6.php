...
function crecimientoVentas($ventasInicial, $ventasFinal) {
    if ($ventasInicial == 0) {
        return $ventasFinal > 0 ? 100 : 0;
    }
    return (($ventasFinal - $ventasInicial) / $ventasInicial) * 100;
}

echo "\nPorcentaje de crecimiento de ventas (primer al último mes):\n";
$mayorCrecimiento = [
    'region' => '',
    'producto' => '',
    'porcentaje' => -INF
...
foreach ($ventas as $region => $productos) {
    echo "$region:\n";
    foreach ($productos as $producto => $ventasProducto) {
        $crecimiento = crecimientoVentas($ventasProducto[0], end($ventasProducto));
        echo "  $producto: " . number_format($crecimiento, 2) . "%\n";
        if ($crecimiento > $mayorCrecimiento['porcentaje']) {
            $mayorCrecimiento = [
                'region' => $region,
                'producto' => $producto,
...
echo "Mayor crecimiento: {$mayorCrecimiento['producto']} en {$mayorCrecimiento['region']} ({$mayorCrecimiento['porcentaje']}%)\n";
?>